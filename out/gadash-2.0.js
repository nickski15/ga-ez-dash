 ...
 *
 * @fileoverview
 * Provides the RealtimeQuery object that simplifies querying the
 * Google Analytics Realtime Reporting API.
 */


/**
 * Realtime Query Builder. This returns a basic object to query the Raltime
 * Reporting API. Developers must override the onSuccess handler to manage
 * the results of the API. Usage:
 *
 * gadash.getCoreQuery({
 *   'query': {
 *     'ids': 'ga:1174',
 ...
 *     Core Reporting API.
 */
gadash.getRealtimeQuery = function(opt_config) {
  return new gadash.GaQuery({
    'onRequestDefault': gadash.core.onRequestDefault,
    'onErrorDefault': gadash.onErrorDefault
  }).setConfig(opt_config);
...
gadash.core.onRequestDefault = function() {
  this.config.actualQuery = gadash.core.getCoreQueryObj(this.config);
  var request = gapi.client.analytics.data.realtime.get(this.config.query);
  request.execute(gadash.util.bindMethod(this, this.callback));
};
